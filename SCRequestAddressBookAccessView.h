//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "UIView.h"

@class NSString, SCButton, TTTAttributedLabel, UILabel, UIScrollView;

@interface SCRequestAddressBookAccessView : UIView
{
    BOOL _access;
    UIScrollView *_scrollView;
    UILabel *_descriptionTextLabel;
    UIView *_contactsNoAccessView;
    SCButton *_giveAccessButton;
    TTTAttributedLabel *_secondaryTextLabel;
    SCButton *_continueButton;
    float _delayTimeLeft;
    NSString *_buttonTitle;
    id <SCRequestAddressBookAccessDelegate> _delegate;
    NSString *_title;
}

@property(retain, nonatomic) NSString *title; // @synthesize title=_title;
@property(nonatomic) BOOL access; // @synthesize access=_access;
@property(nonatomic) __weak id <SCRequestAddressBookAccessDelegate> delegate; // @synthesize delegate=_delegate;
@property(retain, nonatomic) NSString *buttonTitle; // @synthesize buttonTitle=_buttonTitle;
@property(nonatomic) float delayTimeLeft; // @synthesize delayTimeLeft=_delayTimeLeft;
@property(retain, nonatomic) SCButton *continueButton; // @synthesize continueButton=_continueButton;
@property(retain, nonatomic) TTTAttributedLabel *secondaryTextLabel; // @synthesize secondaryTextLabel=_secondaryTextLabel;
@property(retain, nonatomic) SCButton *giveAccessButton; // @synthesize giveAccessButton=_giveAccessButton;
@property(retain, nonatomic) UIView *contactsNoAccessView; // @synthesize contactsNoAccessView=_contactsNoAccessView;
@property(retain, nonatomic) UILabel *descriptionTextLabel; // @synthesize descriptionTextLabel=_descriptionTextLabel;
@property(retain, nonatomic) UIScrollView *scrollView; // @synthesize scrollView=_scrollView;
- (void).cxx_destruct;
- (void)enableContinue:(id)arg1;
- (void)setContinueButtonCountDownTo:(float)arg1;
- (void)layoutSubviews;
- (id)initWithAddressBookAccess:(BOOL)arg1 continueTitle:(id)arg2;

@end

